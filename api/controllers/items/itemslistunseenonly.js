...
module.exports = async function itemslistunseenonly (req, res) {
    ...
    if (Number(page) > 0) {
      ...
      if (rawResult.rows[0].length > 0) {
        ...
        for (var i = 0; i < rawResult.rows[0].length; i++) {
          ...
          var rawResult_user_item_tic = await Users.getDatastore().sendNativeQuery("CALL `user_item_tic_by_user_id`($1,$2,$3);", [user_id, item_id, list_id]);
